...
/*   Updated: 2025/08/01 14:54:34 by lisambet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct s_scene
{
	...
	int						error;
	int						should_exit;
	enum e_selected_object	selected_object;
	void					*sel_obj_ptr;
}							t_scene;
...
double						get_m_projection(t_cylinder *cyl, t_ray r,
								double t);
...
void						resize_selected_object(t_scene *s, int keycode,
								bool *moved);
void						rotate_selected_object_horizontal(t_scene *s,
								int keycode, t_vec obj_up_vec, bool *moved);
void						move_selected_object_vertical(t_scene *s, bool up,
								bool *moved);
void						move_selected_object(t_scene *s, t_vec dir,
								bool *moved, int sign);
void						init_plane_cylinder_vectors(t_scene *s,
								t_vec *obj_right_vec, t_vec *obj_up_vec);
...
void						apply_transform_to_selected(t_scene *s, int keycode,
								bool *moved);
...
bool						hit_cylinder_side(t_cylinder *cyl, t_ray r,
								double *t_side, double m);
bool						hit_cylinder(t_cylinder *cyl, t_ray r,
								double *t_out, int *hit_type);

t_vec						get_hit_object_normal(t_ray r, t_hit_record *rec);
t_vec						get_cylinder_normal(t_cylinder *cyl, t_ray r,
								t_hit_record *rec);
t_color						get_final_pixel_color(t_scene *s, t_ray r,
								t_hit_record *rec);
...
bool						is_blocked(t_scene *s, t_ray shadow_ray,
								double t_max);
t_color						calculate_ambient_color(t_scene *s,
								t_color object_color);
t_color						calculate_direct_color(t_scene *s,
								t_color object_color, t_vec normal,
								t_point intersection_point);

void						check_cylinder_hits(t_scene *s, t_ray r,
								t_hit_record *rec);
void						check_plane_hits(t_scene *s, t_ray r,
								t_hit_record *rec);
void						check_sphere_hits(t_scene *s, t_ray r,
								t_hit_record *rec);
