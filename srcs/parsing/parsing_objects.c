...
/*   Updated: 2025/07/12 03:14:02 by scraeyme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_sphere(char *input, t_data *data)
{
	...
	ft_sphereadd_back(&data->sphere, sphere);
	ft_tabfree(tab, ft_tablen(tab));
}
...
void	parse_plane(char *input, t_data *data)
{
	...
	coords = (t_point){ft_atod(tab[1]), ft_atod(tab[2]), ft_atod(tab[3])};
	vector = vec_normalize((t_point){ft_atod(tab[4]), ft_atod(tab[5]),
			ft_atod(tab[6])});
	rgb = (t_color){ft_atoi(tab[7]), ft_atoi(tab[8]), ft_atoi(tab[9])};
	plane = ft_planenew(coords, vector, rgb);
	ft_planeadd_back(&data->plane, plane);
	ft_tabfree(tab, ft_tablen(tab));
}
