...
/*   Updated: 2025/07/23 14:59:17 by scraeyme         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	print_objects(t_scene scene)
{
	...
	if (scene.amb.enabled)
	{
		printf("\n\033[36m--------------------	AMBIENT		--------------------\n");
		printf("Ratio:		%f\n", scene.amb.i);
		printf("RGB:		%f, %f, %f\n\n\033[0m", scene.amb.color.red, scene.amb.color.green, scene.amb.color.blue);
	}
	...
	if (scene.light.enabled)
	{
		...
		printf("XYZ:		%f, %f, %f\n", scene.light.vtx.x, scene.light.vtx.y, scene.light.vtx.z);
		printf("Ratio:		%f\n", scene.light.i);
		printf("RGB:		%f, %f, %f\n\n\033[0m", scene.light.color.red, scene.light.color.green, scene.light.color.blue);
	}
	i = 1;
	if (scene.spheres)
	{
		...
		while (sphere)
		{
			...
			printf("Diameter:	%f\n", sphere->diameter);
			printf("RGB:		%f, %f, %f\n\n\033[0m", sphere->color.red, sphere->color.green, sphere->color.blue);
			sphere = sphere->next;
			i++;
		}
	}
	...
	if (scene.planes)
	{
		...
		while (plane)
		{
			...
			printf("RGB:		%f, %f, %f\n\n\033[0m", plane->color.red, plane->color.green, plane->color.blue);
			plane = plane->next;
			i++;
		}
	}
	...
	if (scene.cylinders)
	{
		...
		while (cylinder)
		{
			...
			printf("XYZ:		%f, %f, %f\n", cylinder->p0.x, cylinder->p0.y, cylinder->p0.z);
			printf("Axis:		%f, %f, %f\n", cylinder->normal.x, cylinder->normal.y, cylinder->normal.z);
			printf("Diameter:	%f\n", cylinder->diameter);
			printf("Height:		%f\n", cylinder->height);
			printf("RGB:		%f, %f, %f\n\n\033[0m", cylinder->color.red, cylinder->color.green, cylinder->color.blue);
